...
/**
 * @version 1.0
 * @ClassName WxOrderHandleOption
 * @Description 用户可以显示的操作
 * @Author wangdi
 * @Date 2019/10/6 11:54
 **/

public class WxOrderHandleOption {
    boolean cancel; //用户显示取消订单按钮 订单状态101
    boolean delete; //用户显示删除订单按钮  用户取消，系统取消、已退款 、系统收货|用户收货过后
    ...
    public WxOrderHandleOption(Order order) {
        int order_status = order.getOrder_status();
        if(order_status == 101){
            this.cancel = true;
            this.pay = true;
        }
        if(order_status == 102 || order_status == 103||order_status == 203 || order_status == 401 || order_status == 402){
            this.delete = true;
        }
        //商品可以评论个数大于0开启评论 ,
